...
/**
 * Regression unit test suite validating the dynamic infinite anti-sniping framework.
 * Focuses on baseline lifecycle transitions, unbound recursive extensions, and rollback state histories.
 */
...
class AuctionNewTimerTest {
    ...
    void setUp() {
        ...
        auction = Auction.createNewAuction(item, seller, 50L, LocalDateTime.now().minusMinutes(5), DURATION);
    ...
    void createNewAuction_endTimeNullAndDurationStored() {
        Auction fresh = Auction.createNewAuction(item, seller, 50L, LocalDateTime.now(), DURATION);

        assertNull(fresh.getEndTime(), "endTime phải null khi mới tạo (chờ bid đầu tiên)");
        assertEquals(DURATION, fresh.getDurationMinutes(), "durationMinutes phải được lưu đúng");
        assertEquals(Auction.STATUS_OPEN, fresh.getStatus(), "Seller isGood → trạng thái ban đầu phải là OPEN");
    }
    ...
    void createNewAuction_sellerNotGood_statusPending() {
        ...
        Auction pending = Auction.createNewAuction(item, badSeller, 50L, LocalDateTime.now(), DURATION);
    ...
    void firstBid_endTimeSetToNowPlusDuration() {
        ...
        assertFalse(result.newEndTime.isBefore(expectedMin), "endTime phải >= now_before + duration");
        assertFalse(result.newEndTime.isAfter(expectedMax), "endTime phải <= now_after + duration");
    }
    ...
    void firstBid_applyResult_transitionsToRunning() {
        ...
        assertEquals(Auction.STATUS_RUNNING, auction.getStatus(), "Sau bid đầu tiên, status phải là RUNNING");
        assertNotNull(auction.getEndTime(), "Sau bid đầu tiên, endTime không còn null");
    }
    ...
    void antiSniping_extendsBy2Minutes() {
        ...
        assertEquals(expected, result.newEndTime, "endTime phải được gia hạn đúng 2 phút từ endTime cũ");
    }
    ...
    void antiSniping_unlimitedExtensions() {
        ...
        for (int i = 1; i <= 10; i++) {
            ...
            assertEquals(nearEnd.plusSeconds(Auction.ANTI_SNIPING_EXTENSION_SECONDS), result.newEndTime, "Lần gia hạn " + i + ": endTime không đúng");

            auction.applyBidResult(currentBidder, result);
        }

        assertEquals(Auction.STATUS_RUNNING, auction.getStatus(), "Sau 10 lần gia hạn, phiên vẫn phải RUNNING");
    }
    ...
    void antiSniping_noExtensionWhenMoreThan60SecsRemaining() {
        ...
        LocalDateTime farEnd = LocalDateTime.now().plusMinutes(5);
        auction.setEndTime(farEnd);

        Auction.BidResult result = auction.calculateBidResult(bidder2, 2000L);
        assertNotNull(result);

        assertEquals(farEnd, result.newEndTime, "endTime không được gia hạn khi còn hơn 60 giây");
    }
    ...
    void closeIfTimeUp_waitingForBid_notClosed() {
        ...
        assertEquals(Auction.STATUS_WAITING_FOR_BID, auction.getStatus(), "WAITING_FOR_BID không được bị đóng bởi closeAuctionIfTimeIsUp");
    }
    ...
    void revertLastBid_restoresEndTimeAndStatus() {
        ...
        assertEquals(endBefore, auction.getEndTime(), "endTime phải được khôi phục sau revert");
        assertEquals(statusBefore, auction.getStatus(), "status phải được khôi phục sau revert");
    }
